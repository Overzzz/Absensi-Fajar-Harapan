...
function JadwalManager({ token }) {
  ...
  useEffect(() => {
    if(token) {
      refreshData();
      axios.get('https://impressive-caralie-overzzz-8b9026c7.koyeb.app/api/guru', { headers: { Authorization: `Bearer ${token}` }})
        .then(res => setGurus(res.data));
      axios.get('https://impressive-caralie-overzzz-8b9026c7.koyeb.app/api/kelas', { headers: { Authorization: `Bearer ${token}` }})
        .then(res => setKelas(res.data));
    }
  }, [token]);

  const refreshData = () => {
    axios.get('https://impressive-caralie-overzzz-8b9026c7.koyeb.app/api/jadwal', { headers: { Authorization: `Bearer ${token}` }})
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await axios.post('https://impressive-caralie-overzzz-8b9026c7.koyeb.app/api/jadwal', formData, {
  ...
  const handleDelete = async (id) => {
    if(window.confirm('Hapus jadwal ini?')) {
      try {
        await axios.delete(`https://impressive-caralie-overzzz-8b9026c7.koyeb.app/api/jadwal/${id}`, { headers: { Authorization: `Bearer ${token}` }});
        refreshData();
